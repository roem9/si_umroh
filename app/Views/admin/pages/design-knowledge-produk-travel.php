    ...
    <div class="col d-flex justify-content-end mb-3">
      <div>
        <div class="ms-auto my-auto d-none d-md-none d-lg-block">
          <button type="button" class="btn bg-gradient-info btn-sm mb-0 btnModalKnowledgeProduk" data-bs-toggle="modal" data-bs-target="#modalFormData">+&nbsp; Knowledge Produk Baru</a>
        </div>
        <div class="ms-auto my-auto d-block d-md-block d-lg-none">
          <button type="button" class="btn bg-gradient-info btn-sm mb-0 btnModalKnowledgeProduk" data-bs-toggle="modal" data-bs-target="#modalFormData">+&nbsp;</a>
        </div>
      </div>
    </div>
  </div>
  <div class="col-12">
    <div class="card">
      ...
      <div class="card-body">
        ...
        <button type="button" class="btn bg-gradient-info btn-sm mb-0 btnModalKnowledgeProduk w-100" data-bs-toggle="modal" data-bs-target="#modalFormData">+&nbsp; Knowledge Produk Baru</a>
      </div>
    </div>
  </div>
</section>
  ...
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="modalFormDataLabel">Tambah Knowledge  Produk</h5>
        <button type="button" class="btn-close text-dark" data-bs-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <input type="hidden" name="fk_id_produk_travel" id="fk_id_produk_travel" value="<?= $pk_id_produk_travel ?>">
        <form id="formKnowledgeProduk">
          <!-- KALAU SUKSES -->
          ...
          <input type="hidden" name="pk_id_knowledge_produk_travel" id="pk_id_knowledge_produk_travel">
          <div class="data-tipe-knowledge">
            <div class="col-12 mb-3">
              <label for="item">Tipe Knowledge produk</label>
                    ...
                    <div class="card-body">
                      <div class="row">
                        ...
                        <div class="col-10">
                          <h6>Audio</h6>
                          <p class="text-sm">
                            Knowledge produk berupa file audio. File harus berformat mp3.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="card-body">
                      <div class="row">
                        ...
                        <div class="col-10">
                          <h6>File</h6>
                          <p class="text-sm">
                            Knowledge produk berupa file. File harus berformat pdf.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="card-body">
                      <div class="row">
                        ...
                        <div class="col-10">
                          <h6>Gambar</h6>
                          <p class="text-sm">
                            Knowledge produk berupa gambar. File gambar harus berformat jpg, jpeg atau png.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="card-body">
                      <div class="row">
                        ...
                        <div class="col-10">
                          <h6>Video</h6>
                          <p class="text-sm">
                            Knowledge produk berupa video dengan menginput link video dari youtube.
                          </p>
                        </div>
                      </div>
                    </div>
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const btnModalKnowledgeProduk = $(".btnModalKnowledgeProduk");

    // kumpulan even listener
    btnSimpan.on("click", simpanKnowledgeProduk);
    btnModalKnowledgeProduk.on("click", showModalKnowledgeProduk);
    ...
    $(".item").on('click', function() {
      ...
      $("#formKnowledgeProduk #nama_file").val("")

      if (data == 'audio') {
        $(".formItem").hide()
        $("#form-audio").show()
        $(`#formKnowledgeProduk #form-nama-file`).show();
      } else if (data == 'file') {
        $(".formItem").hide()
        $("#form-file").show()
        $(`#formKnowledgeProduk #form-nama-file`).show();
        ...
        $(".formItem").hide()
        $("#form-video").show()
        $(`#formKnowledgeProduk #form-nama-file`).hide();
      } else if (data == 'text') {
        $(".formItem").hide()
        $("#form-text").show()
        $(`#formKnowledgeProduk #form-nama-file`).hide();
  ...
  // kumpulan function
  function showModalKnowledgeProduk(input = true) {
    let form = '#formKnowledgeProduk';
    $('#modalFormDataLabel').html('Tambah KnowledgeProduk');
  ...
  function showData(pk_id_produk_travel) {
    $.ajax({
      url: `<?= base_url()?>/produk/getAllKnowledgeProdukTravel/${pk_id_produk_travel}`,
      type: "GET",
        ...
        if (data.length > 0) {
          for (var i = 0; i < data.length; i++) {
            ...
            if (i == 0) {
              if (data.length == 1) {
                ...
                urutan = `
                          <a href="javascript:void(0)">
                            <span class="badge badge-sm bg-gradient-dark ms-1" onclick="ubahUrutanKnowledgeProduk(${obj.pk_id_knowledge_produk_travel}, ${obj.urutan}, 'turun', ${data[i+1].pk_id_knowledge_produk_travel})">
              ...
              urutan = `
                        <a href="javascript:void(0)">
                          <span class="badge badge-sm bg-gradient-dark ms-1" onclick="ubahUrutanKnowledgeProduk(${obj.pk_id_knowledge_produk_travel}, ${obj.urutan}, 'naik', ${data[i-1].pk_id_knowledge_produk_travel})">
            ...
            if (obj.item == 'text' || obj.item == 'video') {
              action = `
                ...
                <a href='javascript:void(0)' onclick='editKnowledgeProduk(${obj.pk_id_knowledge_produk_travel}, "${obj.item}")'><span class="badge badge-sm bg-gradient-success">edit</span></a>
                ${urutan}
              `
            } else {
              action = `
  ...
  function simpanKnowledgeProduk(e) {
    e.preventDefault();

    let form = '#formKnowledgeProduk';
    let fk_id_produk_travel = $(`#fk_id_produk_travel`).val();
    let pk_id_knowledge_produk_travel = $(`${form} #pk_id_knowledge_produk_travel`).val();
    let item = $(`${form} #item`).val();

    var fd = new FormData();
    fd.append('fk_id_produk_travel', fk_id_produk_travel);
    fd.append('pk_id_knowledge_produk_travel', pk_id_knowledge_produk_travel);
    fd.append('item', item);
    ...
    $.ajax({
      url: "<?= base_url()?>/produk/simpanKnowledgeProdukTravel",
      type: "POST",
      dataType: "json",
      contentType: false,
      processData: false,
  ...
  function editKnowledgeProduk(pk_id_knowledge_produk_travel) {
    $.ajax({
      url: "<?= base_url()?>/produk/getKnowledgeProdukTravel/" + pk_id_knowledge_produk_travel,
      type: "get",
      dataType: "json",
      success: function(response) {
        if (response) {
          showModalKnowledgeProduk(false);

          $('#modalFormData').modal('show');
          $('#modalFormDataLabel').html('Edit Knowledge Produk');
          $(".data-tipe-knowledge").hide();

          $(`#formKnowledgeProduk #pk_id_knowledge_produk_travel`).val(response.pk_id_knowledge_produk_travel);
          $(`#formKnowledgeProduk #fk_id_produk_travel`).val(response.fk_id_produk_travel);
          $(`#formKnowledgeProduk #item`).val(response.item);
          $(`#formKnowledgeProduk #item`).prop('disabled', true);
          ...
          if (response.item == 'text') {
            ...
            $("#form-video").show()
            $(`#formKnowledgeProduk #video`).val(response.data);
          }
        }
      }
  ...
  function hapusKnowledgeProduk(pk_id_knowledge_produk_travel, tipe) {
    Swal.fire({
      title: `Apa Anda yakin akan menghapus knowledge produk ${tipe}?`,
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: "<?= base_url()?>/produk/hapusKnowledgeProdukTravel/" + pk_id_knowledge_produk_travel,
          type: "get",
          success: function(hasil) {
            if (result.isConfirmed) {
              ...
              Toast.fire({
                icon: 'success',
                title: `Berhasil menghapus knowledge produk ${tipe}`
              })
  ...
  function ubahUrutanKnowledgeProduk(pk_id_knowledge_produk_travel, urutan, arah, pk_id_knowledge_produk_travel_other) {
    $.ajax({
      url: "<?= base_url()?>/produk/ubahUrutanKnowledgeProdukTravel",
      type: "POST",
      data: {
        pk_id_knowledge_produk_travel: pk_id_knowledge_produk_travel,
        pk_id_knowledge_produk_travel_other: pk_id_knowledge_produk_travel_other,
        urutan: urutan,
        arah: arah
      },
      success: function(hasil) {
