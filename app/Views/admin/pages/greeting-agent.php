...
<section class="py-3">
    ...
    <div class="col d-flex justify-content-end mb-3">
      <div>
        <div class="ms-auto my-auto d-none d-md-none d-lg-block">
          <button type="button" class="btn bg-gradient-info btn-sm mb-0 btnModalGreeting" data-bs-toggle="modal" data-bs-target="#modalFormData">+&nbsp; Greeting Baru</a>
        </div>
        <div class="ms-auto my-auto d-block d-md-block d-lg-none">
          <button type="button" class="btn bg-gradient-info btn-sm mb-0 btnModalGreeting" data-bs-toggle="modal" data-bs-target="#modalFormData">+&nbsp;</a>
        </div>
      </div>
    </div>
  </div>
  <div class="col-12">
    <div class="card">
      ...
      <div class="card-body">
        <div class="timeline timeline-one-side" data-timeline-axis-style="dotted" id="listGreeting">
        </div>
        <button type="button" class="btn bg-gradient-info btn-sm mb-0 btnModalGreeting w-100" data-bs-toggle="modal" data-bs-target="#modalFormData">+&nbsp; Greeting Baru</a>
      </div>
    </div>
  </div>
</section>
  ...
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="modalFormDataLabel">Tambah Greeting</h5>
      ...
      <div class="modal-body">
        <form id="formData">
          ...
          <input type="hidden" name="pk_id_greeting_agent" id="pk_id_greeting_agent">
          <div class="data-tipe-knowledge">
            <div class="col-12 mb-3">
              <label for="item">Tipe Greeting</label>
                    ...
                    <div class="card-body">
                      <div class="row">
                        ...
                        <div class="col-10">
                          <h6>Audio</h6>
                          <p class="text-sm">
                          Greeting berupa file audio. File harus berformat mp3.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="card-body">
                      <div class="row">
                        ...
                        <div class="col-10">
                          <h6>File</h6>
                          <p class="text-sm">
                            Greeting berupa file. File harus berformat pdf.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="card-body">
                      <div class="row">
                        ...
                        <div class="col-10">
                          <h6>Gambar</h6>
                          <p class="text-sm">
                            Greeting berupa gambar. File gambar harus berformat jpg, jpeg atau png.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="card-body">
                      <div class="row">
                        ...
                        <div class="col-10">
                          <h6>Video</h6>
                          <p class="text-sm">
                            Greeting berupa video dengan menginput link video dari youtube.
                          </p>
                        </div>
                      </div>
                    </div>
          ...
          <div class="data-item">
            ...
            <div class="col-12 mb-3">
              <label for="akses_greeting">Akses Greeting</label>
              <select name="akses_greeting" id="akses_greeting" class="multisteps-form__input form-control">
              ...
              <div class="invalid-feedback" data-id="akses_greeting"></div>
            </div>
          </div>
        </form>
      </div>
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const btnModalGreeting = $(".btnModalGreeting");

    // kumpulan even listener
    btnSimpan.on("click", simpanGreeting);
    btnModalGreeting.on("click", showModalGreeting);
  ...
  // kumpulan function
  function showModalGreeting(input = true) {
    let form = '#formData';
    $('#modalFormDataLabel').html('Tambah Greeting');
  ...
  function showData() {
    $.ajax({
      url: `<?= base_url()?>/greetingagent/getAllGreeting`,
      type: "GET",
      success: function(data) {
        data = JSON.parse(data)

        const listGreeting = $("#listGreeting");
        let obj = {};
        let html = ``;
        let icon = '';
        let content = '';
        ...
        if (data.length > 0) {
          for (var i = 0; i < data.length; i++) {
            ...
            if (i == 0) {
              if (data.length == 1) {
                ...
                urutan = `
                          <a href="javascript:void(0)">
                            <span class="badge badge-sm bg-gradient-dark ms-1" onclick="ubahUrutanGreeting(${obj.pk_id_greeting_agent}, ${obj.urutan}, 'turun', ${data[i+1].pk_id_greeting_agent})">
              ...
              urutan = `
                        <a href="javascript:void(0)">
                          <span class="badge badge-sm bg-gradient-dark ms-1" onclick="ubahUrutanGreeting(${obj.pk_id_greeting_agent}, ${obj.urutan}, 'naik', ${data[i-1].pk_id_greeting_agent})">
            ...
            if (obj.item == 'text' || obj.item == 'video') {
              action = `
                <a href='javascript:void(0)' onclick='hapusGreeting(${obj.pk_id_greeting_agent}, "${obj.item}")'><span class="badge badge-sm bg-gradient-danger me-1">hapus</span></a>
                <a href='javascript:void(0)' onclick='editGreeting(${obj.pk_id_greeting_agent}, "${obj.item}")'><span class="badge badge-sm bg-gradient-success">edit</span></a>
                ${urutan}
              `
            } else {
              action = `
            ...
            if (obj.item == 'video') {
              icon = 'ni-tv-2'
              content = ` <h6 class="text-dark text-sm font-weight-bold mb-2">Video (${obj.akses_greeting})</h6>
              ...
              content = `
                <h6 class="text-dark text-sm font-weight-bold mb-2">File (${obj.akses_greeting})</h6>
                <a href='javascript:void(0)' class='btnPdf' data-title='${obj.data}' data-url='<?= base_url()?>/public/assets/greetingagent/file/${obj.data}' data-bs-toggle="modal" data-bs-target="#pdfModal">
                  <span class="badge badge-sm bg-gradient-secondary">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-file-earmark-arrow-down" viewBox="0 0 16 16">
                      <path d="M8.5 6.5a.5.5 0 0 0-1 0v3.793L6.354 9.146a.5.5 0 1 0-.708.708l2 2a.5.5 0 0 0 .708 0l2-2a.5.5 0 0 0-.708-.708L8.5 10.293V6.5z" />
                      <path d="M14 14V4.5L9.5 0H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2zM9.5 3A1.5 1.5 0 0 0 11 4.5h2V14a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1h5.5v2z" />
              ...
              icon = 'ni-caps-small'
              content = ` <h6 class="text-dark text-sm font-weight-bold mb-2">Text (${obj.akses_greeting})</h6><span class="text-dark">${obj.data}</span>`
            } else if (obj.item == 'audio') {
              icon = 'ni-note-03'
              content = ` <h6 class="text-dark text-sm font-weight-bold mb-2">Audio (${obj.akses_greeting})</h6>
                          <audio controls title='${obj.data}'>
                            <source src="<?= base_url()?>/public/assets/greetingagent/audio/${obj.data}" type="audio/mpeg">
                          </audio>`
            } else if (obj.item == 'image') {
              icon = 'ni-image'
              content = `
                <h6 class="text-dark text-sm font-weight-bold mb-2">Gambar (${obj.akses_greeting})</h6>
                <a href='javascript:void(0)' class='btnImage' data-title='${obj.data}' data-url='<?= base_url()?>/public/assets/greetingagent/img/${obj.data}' data-bs-toggle="modal" data-bs-target="#imageModal">
                  <span class="badge badge-sm bg-gradient-secondary">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-file-earmark-arrow-down" viewBox="0 0 16 16">
                      <path d="M8.5 6.5a.5.5 0 0 0-1 0v3.793L6.354 9.146a.5.5 0 1 0-.708.708l2 2a.5.5 0 0 0 .708 0l2-2a.5.5 0 0 0-.708-.708L8.5 10.293V6.5z" />
                      <path d="M14 14V4.5L9.5 0H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2zM9.5 3A1.5 1.5 0 0 0 11 4.5h2V14a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1h5.5v2z" />
        ...
        listGreeting.html(html);
      }

    });
  }

  function simpanGreeting(e) {
    e.preventDefault();

    let form = '#formData';
    let pk_id_greeting_agent = $(`${form} #pk_id_greeting_agent`).val();
    let item = $(`${form} #item`).val();
    let akses_greeting = $(`${form} #akses_greeting`).val();

    var fd = new FormData();
    fd.append('pk_id_greeting_agent', pk_id_greeting_agent);
    fd.append('item', item);
    fd.append('akses_greeting', akses_greeting);
    ...
    $.ajax({
      url: "<?= base_url()?>/greetingagent/simpanGreeting",
      type: "POST",
      dataType: "json",
      contentType: false,
      processData: false,
  ...
  function editGreeting(pk_id_greeting_agent) {
    $.ajax({
      url: "<?= base_url()?>/greetingagent/getGreeting/" + pk_id_greeting_agent,
      type: "get",
      dataType: "json",
      success: function(response) {
        if (response) {
          showModalGreeting(false);

          $('#modalFormData').modal('show');
          $('#modalFormDataLabel').html('Edit Greeting');
          $(".data-tipe-knowledge").hide();

          $(`#formData #pk_id_greeting_agent`).val(response.pk_id_greeting_agent);
          $(`#formData #item`).val(response.item);
          $(`#formData #item`).prop('disabled', true);
          $(`#formData #akses_greeting`).val(response.akses_greeting);
  ...
  function hapusGreeting(pk_id_greeting_agent, tipe) {
    Swal.fire({
      title: `Apa Anda yakin akan menghapus greeting ${tipe}?`,
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: "<?= base_url()?>/greetingagent/hapusGreeting/" + pk_id_greeting_agent,
          type: "get",
          success: function(hasil) {
            if (result.isConfirmed) {
              ...
              Toast.fire({
                icon: 'success',
                title: `Berhasil menghapus greeting ${tipe}`
              })
  ...
  function ubahUrutanGreeting(pk_id_greeting_agent, urutan, arah, pk_id_greeting_agent_other) {
    $.ajax({
      url: "<?= base_url()?>/greetingagent/ubahUrutanGreeting",
      type: "POST",
      data: {
        pk_id_greeting_agent: pk_id_greeting_agent,
        pk_id_greeting_agent_other: pk_id_greeting_agent_other,
        urutan: urutan,
        arah: arah
      },
      success: function(hasil) {
