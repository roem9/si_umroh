...
<div class="modal fade" id="modalFormDuplikatData" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="modalFormDuplikatDataLabel">Tambah Penjualan</h5>
        <button type="button" class="btn-close text-dark" data-bs-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body" id="formDuplikatData">
        <h6>Data Customer</h6>
        <div class="col-12 mb-3">
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const btnSimpanFormDuplicateData = $("#modalFormDuplikatData #btnSimpan");
    ...
    btnSimpanFormDuplicateData.on("click", saveDataDuplicate);
  ...
  function duplicateData($pk_id_penjualan_produk) {
    ...
    $.ajax({
      ...
      success: function(response) {
        if (response) {
          $('#modalFormDuplikatData').modal('show');
          $('#modalFormDuplikatDataLabel').html('Tambah Penjualan');
          $('.alert-error').hide();
          $('.alert-sukses').hide();
