...
<div class="modal fade" id="modalFormEditPixel" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <input type="hidden" name="fk_id_produk_travel" id="fk_id_produk_travel">
      <div class="modal-body" id="formEditPixel">
        <input type="hidden" name="pk_id_pixel_produk_travel" id="pk_id_pixel_produk_travel">
        <div class="col-12 mb-3">
...
<script>
  ...
  function showData() {
    $('#table-data').DataTable({
      processing: true,
      serverSide: true,
      ajax: `<?= base_url() ?>/aproduk/getListProdukTravel`,
      responsive: {
        details: {
          type: 'column'
        }
      ...
      columns: [{
        ...
        },
        {
          ...
          className: 'text-sm w-1 text-center',
          render: function(data, type, row) {
            return `<a href="javascript:void(0)" data-link="${row.registration_link}" class="copyButton">
            <span class="badge badge-sm bg-gold-custom">salin link</span>
            </a>`;
          }
        },
        {
          ...
          render: function(data, type, row) {
            return `
              <a href="javascript:void(0)" id="${row.pk_id_produk_travel}" class="badge badge-sm bg-gold-custom dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                action
              </a>
              <ul class="dropdown-menu" aria-labelledby="${row.pk_id_produk_travel}">
                <li>
                  <a href="javascript:void(0)" class="dropdown-item" onclick='editData(${row.pk_id_produk_travel})'>
                      detail
                  </a>
                </li>
                <li>
                  <a href="<?= base_url() ?>/agentarea/knowledgeproduktravel/${row.pk_id_produk_travel}" class="dropdown-item">
                      marketing kit
                  </a>
                </li>
                <li>
                  <a href="javascript:void(0)" class="dropdown-item" onclick='historyPixel(${row.pk_id_produk_travel})'>
                      pixel
                  </a>
                </li>
              </ul>
  ...
  function editData($pk_id_produk_travel) {
    let form = '#formData'

    $.ajax({
      url: "<?= base_url() ?>/aproduk/getDataProdukTravel/" + $pk_id_produk_travel,
      type: "get",
      dataType: "json",
      success: function(response) {
        if (response) {
          ...
          $(`${form} #pk_id_produk_travel`).val(response.pk_id_produk_travel);
  ...
  function historyPixel(pk_id_produk_travel) {
    $.ajax({
      url: `<?= base_url() ?>/aproduk/historyPixelProdukTravel/${pk_id_produk_travel}`,
      type: "get",
      dataType: "json",
      success: function(response) {
        $("#modalListPixel").modal('show');

        $(`#fk_id_produk_travel`).val(response.produk.pk_id_produk_travel);
              ...
              <div class="card mb-3 shadow-none border">
                <div class="card-body">
                  ...
                  <div class="d-flex justify-content-end">
                    <a href="javascript:void(0)" class="me-1" onclick='getDataPixel(${pixel.pk_id_pixel_produk_travel})'>
                    ...
                    <a href="javascript:void(0)" onclick='hapusDataPixel(${pixel.pk_id_pixel_produk_travel}, "${pixel.nama_pixel}")'>
  ...
  function getDataPixel(pk_id_pixel_produk_travel) {
    let form = '#formEditPixel';
    ...
    $.ajax({
      url: `<?= base_url() ?>/aproduk/getDataPixelProdukTravel/${pk_id_pixel_produk_travel}`,
      type: 'get',
      dataType: 'json',
      success: function(response) {
        $("#modalListPixel").modal('hide');

        $(`${form} #pk_id_pixel_produk_travel`).val(response.pk_id_pixel_produk_travel);
  ...
  function saveDataPixel(e) {
    ...
    let pk_id_pixel_produk_travel = $(`#pk_id_pixel_produk_travel`).val();
    ...
    let data = {
      pk_id_pixel_produk_travel: pk_id_pixel_produk_travel,
    ...
    $.ajax({
      url: "<?= base_url() ?>/aproduk/saveDataPixelTravel",
      type: "POST",
      data: data,
      dataType: "json",
      success: function(response) {
        if (response.error) {
          ...
          historyPixel(fk_id_produk_travel)
        }
  ...
  function hapusDataPixel(pk_id_pixel_produk_travel, nama_pixel) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: "<?= base_url() ?>/aproduk/hapusDataPixelTravel/" + pk_id_pixel_produk_travel,
          type: "get",
          dataType: "json",
          success: function(response) {
            Toast.fire({
            ...
            let fk_id_produk_travel = $("[name='fk_id_produk_travel']").val();
            historyPixel(fk_id_produk_travel)
