...
<script>
  ...
  function showData() {
    $('#table-data').DataTable({
      processing: true,
      serverSide: true,
      ajax: `<?= base_url()?>/akomisi/getListKomisiPenjualanProdukTravel`,
      responsive: {
        details: {
            type: 'column'
        }
      ...
      columns: [
        ...
        },
        {
          ...
          render: function(data, type, row) {
            return `
              <a href="javascript:void(0)" id="${row.pk_id_komisi_penjualan_produk_travel}" class="badge badge-sm bg-gold-custom dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                action
              </a>
              <ul class="dropdown-menu" aria-labelledby="${row.pk_id_komisi_penjualan_produk_travel}">
                <li>
                  <a href="javascript:void(0)" class="dropdown-item" onclick='getData(${row.pk_id_komisi_penjualan_produk_travel})'>
                      detail
                  </a>
                </li>
              `;
  ...
  function getData($pk_id_komisi_penjualan_produk_travel) {
    ...
    $.ajax({
      url: "<?= base_url()?>/akomisi/getDataKomisiPenjualanProdukTravel/" + $pk_id_komisi_penjualan_produk_travel,
      type: "get",
      dataType: "json",
      success: function(response) {
        if (response) {
          ...
          $(`${form} #pk_id_komisi_penjualan_produk_travel`).val(response.pk_id_komisi_penjualan_produk_travel);
