...
<script>
  ...
  function showData(pk_id_pertemuan_program) {
    $.ajax({
      url: `<?= base_url()?>/programclientarea/getAllLatihanPertemuanKelas/${pk_id_pertemuan_program}`,
      type: "GET",
        ...
        if (data.length > 0) {
          ...
          for (var i = 0; i < data.length; i++) {
            ...
            if (i == 0) {
              if (data.length == 1) {
                ...
                urutan = `
                          <a href="javascript:void(0)">
                            <span class="badge badge-sm bg-gradient-dark ms-1" onclick="ubahUrutanLatihan(${obj.pk_id_latihan_pertemuan_kelas}, ${obj.urutan}, 'turun', ${data[i+1].pk_id_latihan_pertemuan_kelas})">
              ...
              urutan = `
                        <a href="javascript:void(0)">
                          <span class="badge badge-sm bg-gradient-dark ms-1" onclick="ubahUrutanLatihan(${obj.pk_id_latihan_pertemuan_kelas}, ${obj.urutan}, 'naik', ${data[i-1].pk_id_latihan_pertemuan_kelas})">
            ...
            if (obj.item == 'petunjuk' || obj.item == 'soal-pg' || obj.item == 'soal-esai' || obj.item == 'video') {
              action = `
                <a href='javascript:void(0)' onclick='hapusLatihanPertemuan(${obj.pk_id_latihan_pertemuan_kelas}, "${obj.item}")'><span class="badge badge-sm bg-gradient-danger me-1">hapus</span></a>
                <a href='javascript:void(0)' onclick='editLatihanPertemuan(${obj.pk_id_latihan_pertemuan_kelas}, "${obj.item}")'><span class="badge badge-sm bg-gradient-success">edit</span></a>
                ${urutan}
              `
            } else {
              action = `
  ...
  function simpanLatihanPertemuan(e) {
    ...
    $.ajax({
      url: "<?= base_url()?>/programclientarea/simpanLatihanPertemuanKelas",
      type: "POST",
      dataType: "json",
      contentType: false,
      processData: false,
  ...
  function editLatihanPertemuan(pk_id_latihan_pertemuan_kelas) {
    $.ajax({
      url: "<?= base_url()?>/programclientarea/getLatihanPertemuanKelas/" + pk_id_latihan_pertemuan_kelas,
      type: "get",
      success: function(hasil) {
        var obj = $.parseJSON(hasil);
        if (obj.pk_id_latihan_pertemuan_kelas != '') {
          ...
          $(`#formLatihanPertemuan #pk_id_latihan_pertemuan_kelas`).val(obj.pk_id_latihan_pertemuan_kelas);
  ...
  function hapusLatihanPertemuan(pk_id_latihan_pertemuan_kelas, tipe) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: "<?= base_url()?>/programclientarea/hapusLatihanPertemuanKelas/" + pk_id_latihan_pertemuan_kelas,
          type: "get",
          dataType: "json",
          success: function(response) {
            if (result.isConfirmed) {
  ...
  function ubahUrutanLatihan(pk_id_latihan_pertemuan_kelas, urutan, arah, pk_id_latihan_pertemuan_kelas_other) {
    $.ajax({
      url: "<?= base_url()?>/programclientarea/ubahUrutanLatihanKelas",
      type: "POST",
      dataType: "json",
      data: {
        pk_id_latihan_pertemuan_kelas: pk_id_latihan_pertemuan_kelas,
        pk_id_latihan_pertemuan_kelas_other: pk_id_latihan_pertemuan_kelas_other,
        urutan: urutan,
        arah: arah
      },
      success: function(response) {
